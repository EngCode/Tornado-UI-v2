...
var getElement=document.querySelector.bind(document);var getElements=document.querySelectorAll.bind(document);var pageDirection=getComputedStyle(document.body).direction;var addLiveListener=function addLiveListener(selector,event,func){'use strict';if(selector!==null){setInterval(function(){var elements=getElements(selector);Array.from(elements).forEach(function(element){element.addEventListener(event,func)})},1000)}};var getSiblings=function getSiblings(element){'use strict';if(element!==null){return Array.prototype.filter.call(element.parentNode.children,function(sibling){return sibling!==element})}};var getNextSibling=function getNextSibling(element,selector){'use strict';if(element!==null&&selector!==null){var sibling=element.nextElementSibling;while(sibling){if(sibling.matches(selector)){return sibling}}}};var getPrevSibling=function getPrevSibling(element,selector){'use strict';if(element!==null&&selector!==null){var sibling=element.previousElementSibling;while(sibling){if(sibling.matches(selector)){return sibling}}}};var setAttributes=function setAttributes(element,options){'use strict';if(element!==null){Object.keys(options).forEach(function(attr){element.setAttribute(attr,options[attr])})}};function insertAfter(element,reference){'use strict';var elementString=element;if(typeof elementString==="string"&&reference!==null){var range=document.createRange();elementString=range.createContextualFragment(element)}if(reference!==null){reference.parentNode.insertBefore(elementString,reference.nextSibling)}}function insBefore(element,reference){'use strict';var elementString=element;if(typeof elementString==="string"&&reference!==null){var range=document.createRange();elementString=range.createContextualFragment(element)}if(reference!==null){reference.parentNode.insertBefore(elementString,reference)}}function appendIn(reference,element){'use strict';if(typeof element==="string"&&reference!==null){reference.innerHTML+=element}else if(reference!==null){reference.appendChild(element)}}
