...
export const tabsSystem = options => {
    ...
    tui.getElements('.tabs-menu [data-tab]:first-of-type,.tab-content:first-of-type').forEach(function (elements) {
        ...
        if (elements.hasAttribute('data-active-url')) {
            ...
            if (urlHash === '#' + tabID) {
                ...
                tui.getElement('#' + tabID).add('active');
            }
        } else {
            //=== Open the First Tab ===//
    ...
    //=== When Click on Tab
    var tabsBtns = tui.getElements('.tabs-menu [data-tab]');
    Array.from(tabsBtns).forEach(function (thisElement) {
        thisElement.addEventListener('click', function () {
            ...
            var getPanelId = thisElement.getAttribute('data-tab'),
                targtedPanel = tui.getElement('#' + getPanelId);
