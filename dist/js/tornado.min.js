var getElement=document.querySelector.bind(document);var getElements=document.querySelectorAll.bind(document);var pageDirection=getComputedStyle(document.body).direction;var getSiblings=function getSiblings(element){'use strict';return Array.prototype.filter.call(element.parentNode.children,function(sibling){return sibling!==element})};var getNextSibling=function getNextSibling(element,selector){'use strict';var sibling=element.nextElementSibling;while(sibling){if(sibling.matches(selector)){return sibling}}};var getPrevSibling=function getPrevSibling(element,selector){'use strict';var sibling=element.previousElementSibling;while(sibling){if(sibling.matches(selector)){return sibling}}};var setAttributes=function setAttributes(element,options){'use strict';Object.keys(options).forEach(function(attr){element.setAttribute(attr,options[attr])})};function addLiveListener(scope,selector,event,funct){'use strict';setInterval(function(){var selectorParts=selector.split(".");var tag=selectorParts.shift();var className;if(selectorParts.length){className=selectorParts.shift()}if(tag!=""){tag=tag.toUpperCase();var elements=scope.getElementsByTagName(tag)}else{var elements=scope.getElementsByClassName(className);for(var i=0;i<elements.length;i++){if(elements[i][event+"_processed"]===undefined&&(tag==""||elements[i].tagName==tag)){elements[i].addEventListener(event,funct)}}}},1000)}function insertAfter(el,referenceNode){referenceNode.parentNode.insertBefore(el,referenceNode.nextSibling)}
